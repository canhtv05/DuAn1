...
public class repositoriesKhachThue {
    ...
    public ArrayList<model.KhachThue.ModelKhachThue> getAll(){
        ...
        try {
            ...
            while (rs.next()) {                
                ...
                model.KhachThue.ModelKhachThue kt = new ModelKhachThue(maKT,maPT, hoTen, tuoi, gioiTinh, dienThoai, email, diaChi, cccd);
                arrKT.add(kt);
            }
            return arrKT;
        } catch (Exception e) {
    ...
    public int them(model.KhachThue.ModelKhachThue x) {
        sql="insert into KhachThue(MaKT, MaPT, HoTen, Tuoi, GioiTinh, DienThoai, Email, DiaChi , CCCD) values(?,?,?,?,?,?,?,?,?)";
        try {
            con=DBContext.getConnection();
            ps=con.prepareStatement(sql);
            ps.setObject(2, x.getMaPT());
            ps.setObject(3, x.getHoTen());
            ps.setObject(4, x.getTuoi());
    ...
    public int sua(model.KhachThue.ModelKhachThue x){
        sql=" update KhachThue set MaPT=?, HoTen=?, Tuoi=?, GioiTinh=?, DienThoai=?, Email=?, DiaChi=?, CCCD=? where MaKT=?";
        try {
            con=DBContext.getConnection();
            ps=con.prepareStatement(sql);
            ps.setObject(1, x.getMaPT());
            ps.setObject(2, x.getHoTen());
            ps.setObject(3, x.getTuoi());
